...
The reference gateway publishes harness-lifecycle events on the in-process bus shipped by
...
These are AJS-7 PR2/PR3 surface, not part of A2A or AG-UI; they ride the
[bus + SSE substrate](#json-rpc-2-0) that AJS-8 established for operator/federation
tooling. The event-name namespace is `gateway.harness.*` to mark them as
