...
- **`CwdResolutionError`**
- **`NDJSONLineBuffer`** — LF-delimited line buffer for NDJSON process stdout. ACP-over-stdio adapters (Pi, Droid, …) read newline-delimited JSON from a child's stdout. The framing is strictly LF; an optional trailing `\r` i...
- **`ResilientACPProcess`**
...
- **`mergeBinPaths`** — Merge `extraBinPaths` onto a colon-delimited `basePath`, de-duplicating while preserving first-seen order. Existing entries keep their position; new entries append in list order. Empty segments (e....
- **`resolveSessionCwd`**
