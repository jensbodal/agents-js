...
- **`serializeRecords`** — Build the on-disk JSON payload from a record map, preserving v2 shape. Exported so the Phase 2 sync module can write the merged state using the same serializer autoRegister uses — one source of tru...
...
- **`startRegistrySync`** — Wire Phase 1 auto-registration and Phase 2 peer-sync into a gateway startup path. Returns immediately — auto-registration is fire-and-forget. ```ts const sync = startRegistrySync({ name: "my-gatewa...
...
- **`buildSyncPayload`** — Produce the wire payload this gateway would serve at its sync endpoint. Filters out `source="sync"` records — loop prevention on the send side.
