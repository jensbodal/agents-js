...
- **`buildSyncPayload`** — Produce the wire payload this gateway would serve at its sync endpoint. Two filters apply: 1. `source !== "sync"` — loop prevention on the send side; we never re-serve records we received from a pe...
