...
describe("AJS-79 PR3b-2: additive typed failureReasonV2 wire field", () => {
  test("publishReply emits both legacy + V2 when handler provides only legacy kebab", async () => {
    const bus = createGatewayBus();
    const replies: GatewayBusEvent<MatrixBusReplyPayload>[] = [];
    bus.subscribe((event) => {
      if (event.type === MATRIX_REPLY_TOPIC) {
    ...
    const payload = replies[0]?.payload;
    expect(payload?.failureReason).toBe("consumer-unreachable");
    expect(payload?.failureReasonV2).toEqual({ kind: "consumer_unreachable" });
    handle.stop();
  });

  test("publishReply preserves typed V2 verbatim when handler provides both", async () => {
    const bus = createGatewayBus();
    const replies: GatewayBusEvent<MatrixBusReplyPayload>[] = [];
    bus.subscribe((event) => {
      if (event.type === MATRIX_REPLY_TOPIC) {
    ...
    const handle = startMatrixBusConsumer({
      ...
      dispatch: async () => ({
        status: "failure",
        body: "timeout occurred",
        failureReason: "dispatch-timeout",
        failureReasonV2: { kind: "dispatch_timeout", message: "exceeded 30s deadline" },
      }),
    });
    bus.publish({
      id: "evt-both",
    ...
    const payload = replies[0]?.payload;
    expect(payload?.failureReason).toBe("dispatch-timeout");
    expect(payload?.failureReasonV2).toEqual({
      kind: "dispatch_timeout",
      message: "exceeded 30s deadline",
    });
    handle.stop();
  });

  test("dispatch handler throw populates V2 with carried error message", async () => {
    const bus = createGatewayBus();
    const replies: GatewayBusEvent<MatrixBusReplyPayload>[] = [];
    bus.subscribe((event) => {
      if (event.type === MATRIX_REPLY_TOPIC) {
    ...
    const payload = replies[0]?.payload;
    expect(payload?.failureReason).toBe("dispatch-error");
    expect(payload?.failureReasonV2).toEqual({
      kind: "dispatch_error",
  ...
  test("publishReply emits neither failure field on success", async () => {
    const bus = createGatewayBus();
    const replies: GatewayBusEvent<MatrixBusReplyPayload>[] = [];
    bus.subscribe((event) => {
      if (event.type === MATRIX_REPLY_TOPIC) {
    ...
    expect(payload?.failureReasonV2).toBeUndefined();
    handle.stop();
  });
});

describe("convertLegacyFailureReason", () => {
  test("maps dispatch-error → dispatch_error (no message)", () => {
    expect(convertLegacyFailureReason("dispatch-error")).toEqual({ kind: "dispatch_error" });
  });

  test("maps consumer-unreachable → consumer_unreachable (no message)", () => {
    expect(convertLegacyFailureReason("consumer-unreachable")).toEqual({
      kind: "consumer_unreachable",
    });
  });

  test("maps dispatch-timeout → dispatch_timeout (no message)", () => {
    expect(convertLegacyFailureReason("dispatch-timeout")).toEqual({ kind: "dispatch_timeout" });
  });
});

describe("extractFailureReason", () => {
  test("prefers failureReasonV2 over legacy when both present", () => {
    const typedV2 = { kind: "dispatch_error" as const, message: "specific error" };
    const result = extractFailureReason({
      failureReason: "consumer-unreachable",
      failureReasonV2: typedV2,
    });
    expect(result).toBe(typedV2);
  });

  test("falls back to converting legacy when V2 absent", () => {
    const result = extractFailureReason({ failureReason: "dispatch-timeout" });
    expect(result).toEqual({ kind: "dispatch_timeout" });
  });

  test("returns undefined when neither field is set", () => {
    expect(extractFailureReason({})).toBeUndefined();
  });

  test("returns undefined for success payloads with no failure fields", () => {
    expect(
      extractFailureReason({ failureReason: undefined, failureReasonV2: undefined }),
    ).toBeUndefined();
  });
});
