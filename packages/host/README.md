...
- **`createMockAcpController`** — Build a started {ACPSessionController} wired to the repo-root mock ACP agent, packaging the temp-workspace + Node file adapters + empty permission engine/store + `start()` + permission-mode steps t...
...
- **`getMockAgentPath`** — Absolute path to the repo-root `tests/mock-acp-agent.cjs` fixture — the JSON-RPC/NDJSON mock ACP agent spawned as a real `node` subprocess by the runtime smokes. Resolved relative to this module so...
...
- **`makeRecordingMatrixTool`** — Recording {MatrixTool} double — captures every `send` call in `calls` and returns a synthetic `event_id`. The agents-MCP dispatcher requires a Matrix sender even on inbox-only paths; tests assert o...
- **`mintTestJwt`** — Mint an HS256 JWT the way the gateway-side minter does (jose `SignJWT`). Callers pass their OWN signing key via `overrides.key`; the remaining fields default to sensible smoke values so each test o...
- **`normalizeInboxKind`** — Normalise a raw `kind` value (possibly absent, possibly an unrecognised future variant) to a known {InboxKind}. Per spec §7 back-compat: absent / unknown → `"agents_message"`. Use at every consumer...
...
- **`waitFor`** — Poll `predicate` until it holds or the timeout elapses, throwing on timeout. The generic async poller the live-state smokes use to wait for a bridge snapshot / gate event to propagate.
...
- **`MatrixOriginEnvelope`** — AJS-88 / DOT-502 v0.2 — Matrix-origin envelope for bridge-fanout inbox rows. Carries the originating Matrix event metadata onto the durable inbox row so consumers can correlate the inbox audit arti...
...
- **`MockAcpControllerHandle`** — Return shape for {createMockAcpController}.
...
- **`RuntimeSwapResult`**
- **`RuntimeSwitchState`**
- **`SendMessageArgs`** — Arguments for `agents.send_message`. Two call shapes, mutually exclusive: - **Single-target** (`target: string`): single-recipient call shape. Returns a success result with top-level `inbox_message...
...
- **`CorrelationId`**
- **`DispatchHandler`**
- **`GatewayAdditionalFetchHandler`** — A single `additionalFetch` handler: returns a `Response` when it owns the request, or `null` to fall through to the next handler / the A2A server's own routing. Matches `UniversalA2AServerOptions.a...
...
- **`InboxKind`** — AJS-88 / DOT-502 v0.2 — discriminator on durable inbox rows. Distinguishes bridge-fanout-origin rows from native-`agents.send_message`-origin rows. **Open extension:** when deserializing rows from ...
...
- **`SendMessageResult`** — Result of a {AgentsDispatcher.sendMessage} call. AJS-65 route-model: inbox is the durable substrate; matrix is the notification overlay. Success-shape carries flat fields rather than a `delivery` d...
...
- **`createAuditEmitter`**
- **`CURATED_RUNTIME_IDS`**
- **`DEFAULT_DISPATCH_STREAMING_LONG_WARN_MS`** — Default threshold (ms) before a long-running streaming `@` fires a `dispatch-streaming-long-running` audit warning. Mirrors the middleware default. Configurable via `dispatchStreamingLongWarnMs` or...
...
- **`KNOWN_INBOX_KINDS`** — Known-kind set, captured separately from the union so the read-boundary predicate ({normalizeInboxKind}) can be written without inlining an equality chain. Widening the union requires adding to thi...
- **`LEARNING_TEST_TIMEOUT_MS`** — Shared timeout (ms) for the learning-test suites that spawn a real `tests/mock-acp-agent.cjs` subprocess and complete an ACP handshake — slower than the bun:test 5s default. Centralized here so eve...
