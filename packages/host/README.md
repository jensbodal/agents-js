...
- **`convertLegacyFailureReason`** — Map a legacy kebab-case {LegacyFailureReasonKebab} to the typed {DispatchFailureReason} shape. Used by {publishReply} to derive the V2 field when the dispatch handler populated only the legacy fiel...
...
- **`extractFailureReason`** — Read the failure-reason from a {MatrixBusReplyPayload} (or any structurally-compatible carrier) and return a typed {DispatchFailureReason} or `undefined`. Resolution order: 1. If `failureReasonV2` ...
- **`fetchRuntimeModels`**
- **`formatAguiSseFrame`**
...
- **`InboxKind`** — AJS-88 / DOT-502 v0.2 — discriminator on durable inbox rows. Distinguishes bridge-fanout-origin rows from native-`agents.send_message`-origin rows. **Open extension:** consumers that case-switch on...
- **`IssueChallengeResult`** — Result of {ChallengeMintStore.issueChallenge}.
- **`LegacyFailureReasonKebab`** — Legacy kebab-case failure-reason vocabulary used on the `MatrixBusReplyPayload.failureReason` wire field. Three values mirror the three dispatch-transport kinds in {DispatchFailureReason}. **AJS-79...
