...
import { runClaudeReceiverCommand, runClaudeReceiverMcpSendCommand } from "./claude-receiver.ts";
...
function printHelp(output: Pick<NodeJS.WriteStream, "write"> = process.stdout): void {
  output.write(
    `${[
      ...
      "Run `agents-js claude-receiver --help` for receiver env details.",
...
export async function runAgentsJsCli(argv: string[]): Promise<number> {
  ...
  if (command === "claude-receiver") {
    return runClaudeReceiverCommand(rest);
  }
