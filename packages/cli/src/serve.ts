...
function printServeUsage(output: Pick<NodeJS.WriteStream, "write">): void {
  output.write(
    `${[
      ...
      "                                       For multi-harness fleet routing (per-session lane controllers + WS-bridge runtime-switch), use the `agents-js-gateway` binary (from `@agents-js/gateway` in apps/internal-gateway) instead.",
...
async function resolveServeInputs(
  ...
  if ((args.harnesses?.length ?? 0) > 1) {
    output.write(
      `[agents-js] ${args.harnesses?.length ?? 0} harnesses configured (${args.harnesses?.join(", ")}). This standalone CLI uses only the primary ("${args.harnesses?.[0]}"); for multi-harness fleet routing run the \`agents-js-gateway\` binary (from @agents-js/gateway) instead.\n`,
    );
  }
