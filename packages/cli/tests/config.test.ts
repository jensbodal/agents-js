...
describe("agents-js config loading", () => {
  ...
  test("parses and merges defaultModel from user serve config", async () => {
    await withTempWorkspace(async ({ cwd, homeDir, xdgConfigHome }) => {
      const paths = getAgentsJsConfigPaths({
        cwd,
        env: { HOME: homeDir, XDG_CONFIG_HOME: xdgConfigHome },
      ...
      expect(loaded.effectiveConfig.serve?.defaultModel).toBe("user-model");
    });
  });

  test("project config overrides user config for defaultModel", async () => {
    await withTempWorkspace(async ({ cwd, homeDir, xdgConfigHome }) => {
      const paths = getAgentsJsConfigPaths({
        cwd,
        env: { HOME: homeDir, XDG_CONFIG_HOME: xdgConfigHome },
      ...
      await writeFile(
        paths.projectConfigPath,
        JSON.stringify({
          serve: { defaultModel: "project-model" },
        }),
        "utf8",
      );
      ...
      expect(loaded.effectiveConfig.serve?.defaultModel).toBe("project-model");
    });
  });
});
