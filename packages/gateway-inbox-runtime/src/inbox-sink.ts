 ...
 * {@link InboxSinkEmitResult}; a status other than `"emitted"` makes the sink
 * throw so the poller retries. Claude Code's impl is
 * `ClaudeChannelServer.emitChannelMessage`.
 */
export type ChannelEmit = (input: {
  readonly content: string;
  readonly sender: string;
  readonly meta?: Readonly<Record<string, unknown>>;
}) => Promise<InboxSinkEmitResult>;

/**
 * Generic emit outcome. Harness-specific adapters may return richer objects,
 * but the neutral sink only needs to know whether delivery was accepted.
 */
export interface InboxSinkEmitResult {
  readonly status: "emitted" | string;
}
