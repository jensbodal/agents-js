...
- **`searchDocs`** — Grep-style search across markdown files under the given hub-vault root, returning the best-matching line range from each file. Skips `node_modules`, `.git`, `.obsidian`, `.trash`, `dist`, `out`, an...
